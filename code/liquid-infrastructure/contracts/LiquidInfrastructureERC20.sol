...
contract LiquidInfrastructureERC20 is ERC20, ERC20Burnable, Ownable, ERC721Holder, ReentrancyGuard {
    ...
    //###
    uint256 private constant PRECISION_FACTOR = 10 ** 3;
    ...
    function _beforeTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (!(to == address(0))) {
            require(isApprovedHolder(to), "receiver not approved to hold the token"); //HolderAllowlist
        }
        if (from == address(0) || to == address(0)) {
            //we are about to mint or burn
            _beforeMintOrBurn();
        }
        //exists = true (has token) ; false: has no tokens
        bool exists = (this.balanceOf(to) != 0);
        if (!exists) {
            //@audit-ok to potentially pushed twice: to is added to holderArr => to has no tokens yet & is added again
            holders.push(to);
        }
    }
    ...
    function _beforeMintOrBurn() internal view {
        require(!_isPastMinDistributionPeriod(), "must distribute before minting or burning");
    }
    ...
    function _afterTokenTransfer(address from, address to, uint256 amount) internal virtual override {
        ...
        if (!stillHolding) {
            //@audit-ok unbounded loop
            for (uint i = 0; i < holders.length; i++) {
                if (holders[i] == from) {
                    holders[i] = holders[holders.length - 1];
                    holders.pop();
                    //@audit-ok missing break
                }
            }
        }
    }
    ...
    //create liqNFT
    //transfer liqNFT to liqERC20 & add it to ManagedNFTs
    //transfer reward tokens to NFT
    //call withdrawFromAllManagedNFTs => transfers reward tokens from liqNFT to liqERC20
    ...
    function distribute(uint256 numDistributions) public nonReentrant {
        ...
        //@audit missing check for LockedForDistribution
        if (!LockedForDistribution) {
            require(_isPastMinDistributionPeriod(), "MinDistributionPeriod not met");
            _beginDistribution(); //nextDistributionRecipient = 0 && fill arr: erc20EntitlementPerUnit
        }
        ...
        for (i = nextDistributionRecipient; i < limit; i++) {
            ...
            if (isApprovedHolder(recipient)) {
                ...
                for (uint j = 0; j < distributableERC20s.length; j++) {
                    ...
                    //PRECISION_FACTOR
                    //@audit-ok avoid transfer in loop => if it fails for one it fails for all => DOS => receiver is a contract
    ...
    function _beginDistribution() internal {
        ...
        for (uint i = 0; i < distributableERC20s.length; i++) {
            ...
            //@audit supply is larger than balance => returns alaways 0
    ...
    function _endDistribution() internal {
        require(LockedForDistribution, "cannot end distribution when not locked");
        delete erc20EntitlementPerUnit;
     ...
     */
    function mintAndDistribute(address account, uint256 amount) public onlyOwner {
